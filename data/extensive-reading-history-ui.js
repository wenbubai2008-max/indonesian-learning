(function(){
  ...
  function loadFreshSource(path){return new Promise(function(resolve,reject){var s=document.createElement('script');s.src=base()+path+'?v='+Date.now();s.onload=function(){s.remove();resolve();};s.onerror=function(){s.remove();reject(new Error('最新内容读取失败：'+path));};document.head.appendChild(s);});}
  async function refreshLatestSources(){try{await loadFreshSource('data/extensive-reading-data.js');}catch(e){}try{await loadFreshSource('data/extensive-reading-history.js');}catch(e){}}
  function records(){
  ...
  function doSpeak(text,btn){var v=getIndonesianVoice();if(!v){if(typeof window.speak==='function'){window.speak(text);return;}alert('当前设备没有可用的印尼语语音。');return;}if(!('speechSynthesis' in window)||!window.SpeechSynthesisUtterance){if(typeof window.speak==='function'){window.speak(text);return;}alert('当前浏览器不支持网页朗读，请用 Chrome / Edge / Safari 最新版打开。');return;}var synth=window.speechSynthesis;try{synth.cancel();}catch(e){}var u=new SpeechSynthesisUtterance(text);u.lang='id-ID';u.voice=v;u.rate=.92;u.pitch=1;u.volume=1;activeUtterance=u;var old=btn?btn.textContent:'';if(btn){btn.textContent='⏹ 停止朗读';btn.dataset.speaking='1';}u.onend=u.onerror=function(){if(btn){btn.textContent=old||'🔊 朗读本段';delete btn.dataset.speaking;}activeUtterance=null;};try{synth.speak(u);if(synth.paused)synth.resume();}catch(e){if(btn){btn.textContent=old||'🔊 朗读本段';delete btn.dataset.speaking;}if(typeof window.speak==='function')window.speak(text);else alert('朗读启动失败，请刷新页面后再试。');}}
  ...
  window.openExtensiveV2=async function(){if(window.setupPages)window.setupPages();if(window.go)go('extensive');var body=document.getElementById('extensiveBody');if(body)body.innerHTML='<div class="loading">正在读取最新泛读…</div>';await refreshLatestSources();currentIndex=0;render();};
