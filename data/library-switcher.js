(function(){
  ...
  function localUnknownWords(){
    ...
    return Object.values(m).map(x=>({word:x.word,display:x.display||x.word,cn:x.cn||'',en:x.en||'',root:x.root||'',scene:(x.contexts&&x.contexts.length)?x.contexts[x.contexts.length-1]:'',note:'遇到 '+(x.times_seen||1)+' 次'+(x.source_date?' · '+x.source_date:'')+(x.session?' · '+x.session:''),contexts:x.contexts||[],times_seen:x.times_seen||1,source:'电脑新增'}));
  }
  ...
  async function translateZh(word){
    ...
    try{const url='https://translate.googleapis.com/translate_a/single?client=gtx&sl=id&tl=zh-CN&dt=t&q='+encodeURIComponent(w);const r=await fetch(url,{method:'GET',mode:'cors',cache:'no-store'});if(!r.ok)return '';const data=await r.json();return Array.isArray(data?.[0])?data[0].map(x=>Array.isArray(x)?x[0]:'').join('').trim():'';}catch(e){return ''}
  }
  async function fillMissingMeanings(){
    ...
    for(const [key,item] of Object.entries(map)){
      ...
      if(shared&&!missingMeaning(shared.cn)){item.cn=shared.cn;map[key]=item;changed=true;continue;}
      const attemptKey=normWord(item.word);if(meaningAttempts.has(attemptKey))continue;meaningAttempts.add(attemptKey);
      const zh=await translateZh(item.word);if(zh){item.cn=zh;map[key]=item;changed=true;}
    }
    if(changed){localStorage.setItem('indo_unknown_words',JSON.stringify(map));if(document.getElementById('librarySelect')?.value==='unknown'){DB=sourceFor('unknown');FILTER=DB.slice();idx=restoreIndex('unknown',FILTER);rebuildCategories();renderVocab();decorateCard();document.getElementById('vocabCount').textContent=DB.length;document.getElementById('vocabTag').textContent='陌生词汇 '+DB.length+' 词';document.getElementById('dbStatus').textContent='陌生词汇 · '+DB.length+' 词';}}
  }

  function sourceFor(key){if(key==='daily')return uniqueByWord(window.DAILY_VOCAB_DB||[]);if(key==='unknown')return uniqueByWord(unknownWords());return uniqueByWord(window.EMBEDDED_DB||[]);}
  ...
  function decorateCard(){
    ...
    let meaning=flash.querySelector('.meaning');if(!meaning)return;
    const oldScene=meaning.querySelector('[data-daily-example]');if(oldScene)oldScene.remove();
    const ex=String(x.example||x.scene||'').trim(),exCn=String(x.example_cn||x.scene_cn||'').trim();
    if(ex){
      const box=document.createElement('div');box.dataset.dailyExample='1';box.style.marginTop='16px';box.style.lineHeight='1.65';
      box.innerHTML='<div style="font-size:17px">'+esc(ex)+'</div>'+(exCn?'<div class="muted" style="margin-top:4px">'+esc(exCn)+'</div>':'');
  ...
  function setLibrary(key){
    ...
    document.getElementById('vocabCount').textContent=DB.length;document.getElementById('vocabTag').textContent=labelFor(key)+' '+DB.length+' 词';document.getElementById('dbStatus').textContent=labelFor(key)+' · '+DB.length+' 词';
    if(DB.length){renderVocab();decorateCard();saveCurrentProgress();}else{document.getElementById('vocabBox').innerHTML='<div class="empty">这个词库目前还没有词。</div>';}
    updateStats();localStorage.setItem('selected_vocab_library',key);if(key==='unknown')setTimeout(fillMissingMeanings,0);
  }
  ...
  function install(){
    ...
    let select=document.getElementById('librarySelect');if(!select){select=document.createElement('select');select.id='librarySelect';select.onchange=function(){saveCurrentProgress();setLibrary(this.value)};toolbar.insertBefore(select,toolbar.firstChild);}
    refreshOptions();loadDB=function(){setLibrary(document.getElementById('librarySelect')?.value||'top1000')};window.loadDB=loadDB;window.speak=robustSpeak;speak=robustSpeak;
    ...
    const originalMark=window.mark;const unknownAwareMark=function(v){const lib=document.getElementById('librarySelect')?.value;if(lib!=='unknown')return originalMark(v);const x=current();if(!x)return;let m=memory();m[x.word]=v;localStorage.setItem('indo_mem',JSON.stringify(m));if(v==='know')removeLocalUnknown(x.word);updateStats();renderReview();refreshOptions();setLibrary('unknown');};window.mark=unknownAwareMark;mark=unknownAwareMark;
    setLibrary(localStorage.getItem('selected_vocab_library')||'top1000');
  }
  window.switchVocabLibrary=setLibrary;window.getUnfamiliarVocabulary=unknownWords;window.refreshUnknownLibrary=function(){refreshOptions();if(document.getElementById('librarySelect')?.value==='unknown')setLibrary('unknown')};window.addEventListener('unknown-vocab-changed',function(){window.refreshUnknownLibrary&&window.refreshUnknownLibrary()});window.addEventListener('beforeunload',saveCurrentProgress);window.addEventListener('load',function(){setTimeout(install,0)});
})();
