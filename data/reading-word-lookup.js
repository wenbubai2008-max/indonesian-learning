(function(){
  ...
  function sentenceFrom(node,raw){
    ...
    const rest=text.slice(p+String(raw).length);const m=rest.match(/[.!?。！？]/);if(m)b=p+String(raw).length+m.index+1;
  ...
  function saveUnknown(hit,node){
    ...
    const now=new Date().toISOString(),m=unknownMap(),src=sourceInfo(node),context=sentenceFrom(node,hit.word);
    if(m[key]){
      m[key].last_seen=now;m[key].times_seen=(m[key].times_seen||1)+1;
      if(hit.cn&&(!m[key].cn||m[key].cn==='暂无释义'))m[key].cn=hit.cn;
      if(context&&!m[key].contexts?.includes(context))m[key].contexts=(m[key].contexts||[]).concat(context).slice(-5);
    }else{
      m[key]={word:hit.base||key,display:hit.word,cn:hit.cn||'暂未查到释义',en:hit.en||'',root:hit.root||'',first_seen:now,last_seen:now,times_seen:1,source:src.source,source_date:src.date,session:src.session,contexts:context?[context]:[]};
    }
  ...
  function show(raw,rect,node){
    ...
    hide();ensureStyle();popup=document.createElement('div');popup.className='rw-popup';
    ...
    const cnEl=popup.children[1],btn=popup.querySelector('.rw-add');
    btn.addEventListener('mousedown',e=>e.stopPropagation());
    btn.addEventListener('click',async e=>{
      e.stopPropagation();if(isUnknown(hit.base||hit.word))return;
      btn.disabled=true;btn.textContent='正在加入…';
      await ensureMeaning(hit,cnEl);saveUnknown(hit,node);
      btn.disabled=false;btn.textContent='✓ 已加入';btn.classList.add('added');
    });
    document.body.appendChild(popup);
    if(!hit.cn)ensureMeaning(hit,cnEl);
