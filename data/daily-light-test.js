(function(){
  ...
  function tokenButtons(tokens){return shuffleTokens(tokens).map(t=>`<button class="dailyFixChoice dailyLightToken" data-token="${esc(t)}" onclick="dailyLightPick(this)">${esc(t)}</button>`).join('')}
  function orderPrompt(it){
    const cn=String(it.answer_cn||'').trim();
    if(cn)return `按照中文排成自然的印尼语句子：${cn}`;
    const p=String(it.prompt||'').trim();
    return p||'按照中文排成自然的印尼语句子';
  }
  function order(it,i){return `<div class="dailyFixItem dailyLightOrder" data-order="[]"><b>${i+1}. ${esc(orderPrompt(it))}</b><div class="dailyLightTokenWrap" data-tokens='${esc(JSON.stringify(it.tokens||[]))}' style="display:flex;gap:7px;flex-wrap:wrap;margin-top:10px">${tokenButtons(it.tokens||[])}</div><div class="dailyLightOrderOut" style="min-height:42px;margin-top:10px;padding:9px 11px;background:#f8faff;border-radius:9px"></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="dailyFixToggle" onclick='dailyLightOrderCheck(this,${JSON.stringify(it.answer||'')},${JSON.stringify(it.answer_cn||'')})'>检查顺序</button><button class="dailyFixToggle" onclick="dailyLightOrderReset(this)">重排</button></div><div class="dailyFixFeedback"></div></div>`}
  ...
  window.dailyLightOrderReset=function(btn){
    const box=btn.closest('.dailyLightOrder');box.dataset.order='[]';box.querySelector('.dailyLightOrderOut').textContent='';
    const wrap=box.querySelector('.dailyLightTokenWrap');
    if(wrap){let tokens=[];try{tokens=JSON.parse(wrap.dataset.tokens||'[]')}catch(e){}wrap.innerHTML=tokenButtons(tokens);}
    const fb=box.querySelector('.dailyFixFeedback');fb.classList.remove('show');fb.textContent='';
  };
  ...
  window.openDaily=async function(s,d){await base(s,d);const date=(document.getElementById('dailyMeta')?.textContent||d||'').trim();if(!date)return;try{const x=await fetchJSON(`data/daily/${date}-${s}.json`);if(!x.daily_test)return;const body=document.getElementById('dailyBody');if(!body||body.querySelector('.dailyLightTest'))return;const done=body.lastElementChild;const wrap=document.createElement('div');wrap.innerHTML=render(x.daily_test,x);const node=wrap.firstElementChild;if(done)body.insertBefore(node,done);else body.appendChild(node)}catch(e){}}
})();