(function(){
  ...
  style.textContent=`
    ...
    .hv-answer{display:none;background:#f7f8fb;border-radius:11px;padding:11px;margin-top:9px;line-height:1.65}.hv-open .hv-answer{display:block}.hv-toggle{border:0;background:#eef1f6;border-radius:9px;padding:7px 10px;margin-top:9px;font-weight:700;cursor:pointer}
  ...
  function chips(x){const a=[];if(x.progress){a.push(`今日新增 ${x.progress.new}`);a.push(`累计 ${x.progress.cumulative}`);if(x.progress.range)a.push(`编号 ${x.progress.range}`)}if(x.vocab?.length)a.push(`词汇 ${x.vocab.length}`);if(x.sentences?.length)a.push(`句子 ${x.sentences.length}`);if(x.reading)a.push('阅读 1');if(x.quiz?.length)a.push(`小测 ${x.quiz.length}`);if(x.output)a.push('输出 1');return `<div class="hv-chips">${a.map(t=>`<span class="hv-chip">${esc(t)}</span>`).join('')}</div>`}
