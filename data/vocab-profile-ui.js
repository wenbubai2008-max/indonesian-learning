(function(){
  ...
  function ensureStyle(){
    ...
    s.textContent=`
      ...
      @media(max-width:650px){.vpGrid,.vpMiniStats{grid-template-columns:repeat(2,1fr)}.vpFoot{flex-direction:column;gap:3px}.vpAbility,.vpWeakList,.vpAdviceGrid{grid-template-columns:1fr}.vpDetailHero{flex-direction:column;gap:8px}}
    `;
  ...
  function mount(data){
    ...
    box.innerHTML='<div class="vpHead"><div><h3>个人词汇画像</h3><div class="muted">只统计正式学过的词，不把词库筛选或泛读路过当成掌握。</div></div><span class="pill">画像</span></div><div class="vpGrid"><div class="vpMetric"><b>'+data.taught_total+'</b><span>已正式学习</span></div><div class="vpMetric"><b>'+data.confirmed_mastered+'</b><span>已确认掌握</span></div><div class="vpMetric"><b>'+data.needs_reinforcement+'</b><span>待强化</span></div><div class="vpMetric"><b>'+data.unverified+'</b><span>尚待验证</span></div></div><div class="vpProgress"><i style="width:'+Math.max(0,Math.min(100,Number(data.mastery_percent)||0))+'%"></i></div><div class="vpFoot"><span>总体确认掌握进度 '+data.mastery_percent+'%</span><span>当前重点弱词 '+data.focus_total+' 个</span></div>'+(focus.length?'<div class="vpFocus"><b>当前优先：</b> '+focus.map(x=>'<span class="vpChip" title="'+esc(x.cn||'')+'">'+esc(x.word)+'</span>').join('')+'</div>':'')+'<button class="vpOpen" type="button" id="openVocabProfile">查看完整词汇画像 →</button>';
  ...
  function renderDetail(){
    ...
    body.innerHTML='<div class="vpDetailHero"><div><h2>个人词汇画像</h2><p>把学习结果、训练错误和后续验证汇总成一个持续更新的词汇能力档案。</p></div><span class="pill">第二版结构</span></div>'+
      ...
      '<section class="vpPanel '+(activeTab==='overview'?'active':'')+'" data-vp-panel="overview"><div class="vpAbility"><div class="vpAbilityCard"><span>主动词汇</span><b>'+active+'</b><small>需要无提示主动提取，并经过间隔验证。当前证据不足时保持“待验证”。</small></div><div class="vpAbilityCard"><span>被动识别词汇</span><b>'+passive+'</b><small>需要在词义或听觉识别中有稳定证据；单纯阅读曝光不计入。</small></div></div><div class="vpSection"><h3>已学习词状态</h3><div class="vpMiniStats"><div><b>'+data.taught_total+'</b><span>正式学习</span></div><div><b>'+data.confirmed_mastered+'</b><span>确认掌握</span></div><div><b>'+data.needs_reinforcement+'</b><span>待强化</span></div><div><b>'+data.unverified+'</b><span>尚待验证</span></div></div></div></section>'+
