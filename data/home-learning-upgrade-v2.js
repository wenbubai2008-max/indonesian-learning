(function(){
  ...
  function setupHome(){
    ...
    if(!tag)return;
    var count=(window.DAILY_VOCAB_DB||[]).length||(window.EMBEDDED_DB||[]).length||0;
    if(count)tag.textContent=count+' 个词';
  }
  ...
  style();
  if('requestIdleCallback' in window)requestIdleCallback(setupHome,{timeout:500});else setTimeout(setupHome,0);
})();