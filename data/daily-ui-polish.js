(function(){
  ...
  async function latestAvailableDate(session,requested){
    try{
      ...
      const previous=rows.filter(function(x){return x.date<requested&&x[session];}).sort(function(a,b){return a.date<b.date?1:-1;})[0];
      return previous?previous.date:requested;
    }catch(e){
      return requested;
    }
  }

  function installDailyFallback(){
    const original=window.openDaily;
    if(typeof original!=='function'||original.__previousLessonFallback)return;
    const wrapped=async function(session,date){
      if(date)return original(session,date);
      const requested=jakartaToday();
      const resolved=await latestAvailableDate(session,requested);
      return original(session,resolved);
    };
    wrapped.__previousLessonFallback=true;
