(function(){
  ...
  function current(){const a=arr();if(!a.length)return null;const i=((pos()%a.length)+a.length)%a.length;return a[i]||a[0]}
  function isBipa(){return !!window.VocabController?.isBipa?.()}
  function bipaLevel(){return window.VocabController?.bipaLevel?.()||''}
  function rawBipa(word){const rows=(window.BIPA_VOCAB_RAW&&window.BIPA_VOCAB_RAW[bipaLevel()])||[];const r=rows.find(x=>Array.isArray(x)&&norm(x[0])===norm(word));if(!r)return null;return {word:r[0]||'',cn:r[1]||'',en:r[2]||'',root:r[3]||'',theme:r[5]||'',sab:String(r[8]||'').trim().toUpperCase()}}
  function info(x){const r=isBipa()?rawBipa(x&&x.word):null;return {word:(r&&r.word)||(x&&x.word)||'',cn:(r&&r.cn)||(x&&x.cn)||'',en:(r&&r.en)||(x&&x.en)||'',root:(r&&r.root)||(x&&x.root)||'',theme:(r&&r.theme)||(x&&x.theme)||'',sab:(r&&r.sab)||String((x&&(x.sab||x.grade||x.priority))||'').toUpperCase(),example:(x&&(x.example||x.scene))||'',exampleCn:(x&&(x.example_cn||x.scene_cn))||'',note:(x&&x.note)||''}}
  ...
  function installStyle(){
    ...
    st=document.createElement('style');st.id='vocabUnifiedStyle20260917';st.textContent=`
      ...
      #vocabBox>.flash,#vocabBox>.cleanFlash,#vocabBox>.bipaV7Card,#vocabBox>.bipaStableCard{display:none!important}
  ...
  window.vocabUnifiedSpeak=function(text){
    ...
    if(window.VocabController&&typeof window.VocabController.speak==='function')return window.VocabController.speak(text);
    try{if(audio){audio.pause();audio=null}}catch(e){}try{if(window.speechSynthesis)window.speechSynthesis.cancel()}catch(e){}
    try{if(window.speechSynthesis&&typeof SpeechSynthesisUtterance!=='undefined'){const u=new SpeechSynthesisUtterance(text);u.lang='id-ID';u.rate=.88;window.speechSynthesis.speak(u);return}}catch(e){}
  ...
  function render(){
    installStyle();const box=$('vocabBox');if(!box)return;const a=arr(),x=current();
    if(!a.length||!x){const msg=window.VocabController?.emptyMessage?.()||'没有匹配词汇';box.innerHTML='<div class="empty">'+esc(msg)+'</div>';return}
    if(($('mode')?.value||'flash')==='quiz'){
      const d=info(x),others=a.filter(y=>y.word!==x.word&&(info(y).cn||info(y).en)).sort(()=>Math.random()-.5).slice(0,3),opts=[x,...others].sort(()=>Math.random()-.5);
      box.innerHTML='<div class="q">“'+esc(d.word)+'” 最接近哪个意思？ <button class="sound" type="button" onclick="vocabUnifiedSpeak('+JSON.stringify(d.word).replace(/"/g,'&quot;')+')">🔊</button></div><div class="opts">'+opts.map(o=>{const z=info(o);return '<button class="opt" onclick="answerQuiz(this,'+JSON.stringify(o.word===x.word)+')">'+esc(z.cn||z.en||'—')+'</button>'}).join('')+'</div><div class="actions"><button class="primary" onclick="vocabUnifiedNext()">下一题</button></div>';return;
    }
    ...
    box.innerHTML='<div id="vocabUnifiedCard" class="vocabUnifiedCard'+(isBipa()?' vocabUnifiedBipa':'')+'"><div class="vocabUnifiedProgress">第 '+p+' / '+a.length+' 个</div>'+topic+'<div class="vocabUnifiedCore">'+badge+'<div class="vocabUnifiedWordLine"><div class="vocabUnifiedWord">'+esc(d.word)+'</div><button class="vocabUnifiedSound" type="button" onclick="event.preventDefault();event.stopPropagation();vocabUnifiedSpeak('+JSON.stringify(d.word).replace(/"/g,'&quot;')+')">🔊</button></div><div class="vocabUnifiedMeaning"><div class="vocabUnifiedCn">'+esc(d.cn||'暂无中文')+'</div>'+en+root+example+exampleCn+note+'</div></div></div><div class="vocabUnifiedActions"><button class="nav" type="button" onclick="vocabUnifiedPrev()">上一个</button><button class="nav" id="vocabUnifiedFlipBtn" type="button" onclick="vocabUnifiedFlip()">翻卡</button><button class="next" type="button" onclick="vocabUnifiedNext()">下一个</button></div><div class="vocabUnifiedMemory"><button class="know" type="button" onclick="vocabUnifiedMark(\'know\')">会了</button><button class="fuzzy" type="button" onclick="vocabUnifiedMark(\'fuzzy\')">模糊</button><button class="dont" type="button" onclick="vocabUnifiedMark(\'dont\')">不会</button></div>';
  }
  window.vocabUnifiedFlip=function(){const c=$('vocabUnifiedCard'),b=$('vocabUnifiedFlipBtn');if(!c)return;c.classList.toggle('revealed');const opened=c.classList.contains('revealed'),m=c.querySelector('.vocabUnifiedMeaning');if(m)m.style.display=opened?'block':'none';if(b)b.textContent=opened?'收起':'翻卡'};
  window.vocabUnifiedPrev=function(){window.VocabController?.move?.(-1)};
  window.vocabUnifiedNext=function(){window.VocabController?.move?.(1)};
  window.vocabUnifiedMark=function(v){window.VocabController?.mark?.(v)};
  function enforce(){
    installStyle();window.renderVocab=render;try{renderVocab=render}catch(e){}
    if(window.VocabController){window.applyFilter=()=>window.VocabController.rebuild(true);try{applyFilter=window.applyFilter}catch(e){}window.showKnownWords=()=>window.VocabController.setView('known');try{showKnownWords=window.showKnownWords}catch(e){}window.showReviewWords=()=>window.VocabController.setView('review')}
    const sec=$('vocab');if(sec)sec.classList.remove('bipaStable','bipaSwitching');document.querySelectorAll('#vocabBox>.flash,#vocabBox>.cleanFlash,#vocabBox>.bipaV7Card,#vocabBox>.bipaStableCard').forEach(n=>n.remove());
  }
  let fixing=false;const obs=new MutationObserver(()=>{if(fixing)return;const box=$('vocabBox');if(!box)return;if(box.querySelector(':scope>.flash,:scope>.cleanFlash,:scope>.bipaV7Card,:scope>.bipaStableCard')){fixing=true;render();queueMicrotask(()=>{fixing=false})}});
  function boot(){enforce();const box=$('vocabBox');if(box)obs.observe(box,{childList:true,subtree:false});window.VocabController?.refresh?.(false);render()}
  if(document.readyState==='complete')setTimeout(boot,0);else window.addEventListener('load',()=>setTimeout(boot,0),{once:true});
  window.addEventListener('vocab-controller-ready',()=>setTimeout(()=>{enforce();window.VocabController?.refresh?.(false);render()},0));
  window.addEventListener('vocab-library-ready',()=>setTimeout(render,0));
})();
