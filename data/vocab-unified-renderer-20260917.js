(function(){
  ...
  function all(){ try{return (DB || []).slice()}catch(e){return []} }
  function at(){ const a=arr(); if(!a.length)return null; let i=0; try{i=Number(idx||0)}catch(e){}; i=((i%a.length)+a.length)%a.length; return a[i]||a[0]; }
  function libText(){ const s=$('librarySelect'); return s&&s.selectedIndex>=0 ? String(s.options[s.selectedIndex]?.textContent||'') : ''; }
  function isBipa(){ return /BIPA\s*[（(]?\s*(A1|A2|B1|B2)/i.test(libText()); }
  ...
  function installStyle(){
    ...
    st=document.createElement('style'); st.id='vocabUnifiedStyle20260917'; st.textContent=`
      ...
      #vocab .vocabUnifiedCn{font-size:27px;font-weight:850;color:#5d687e;line-height:1.45}.vocabUnifiedEn{font-size:20px;color:#778197;margin-top:11px;line-height:1.45}.vocabUnifiedRoot{font-size:16px;color:#8a94a8;font-weight:750;margin-top:22px}
  ...
  function render(){
    ...
    const a=arr(),x=at(); if(!a.length||!x){box.innerHTML='<div class="empty">没有匹配词汇</div>';return;}
    ...
    const d=info(x), pos=(Math.max(0,Number((typeof idx!=='undefined'&&idx)||0))%a.length)+1;
    ...
    box.innerHTML='<div id="vocabUnifiedCard" class="vocabUnifiedCard"><div class="vocabUnifiedProgress">第 '+pos+' / '+a.length+' 个</div>'+topic+'<div>'+badge+'<div class="vocabUnifiedWordLine"><div class="vocabUnifiedWord">'+esc(d.word)+'</div><button class="vocabUnifiedSound" type="button" onclick="event.preventDefault();event.stopPropagation();vocabUnifiedSpeak('+JSON.stringify(d.word).replace(/"/g,'&quot;')+')">🔊</button></div><div class="vocabUnifiedMeaning"><div class="vocabUnifiedCn">'+esc(d.cn||'暂无中文')+'</div><div class="vocabUnifiedEn">'+esc(d.en||'')+'</div>'+root+'</div></div></div><div class="vocabUnifiedActions"><button class="nav" type="button" onclick="vocabUnifiedPrev()">上一个</button><button class="nav" id="vocabUnifiedFlipBtn" type="button" onclick="vocabUnifiedFlip()">翻卡</button><button class="next" type="button" onclick="vocabUnifiedNext()">下一个</button></div><div class="vocabUnifiedMemory"><button class="know" type="button" onclick="vocabUnifiedMark(\'know\')">会了</button><button class="fuzzy" type="button" onclick="vocabUnifiedMark(\'fuzzy\')">模糊</button><button class="dont" type="button" onclick="vocabUnifiedMark(\'dont\')">不会</button></div>';
  }
  ...
  window.vocabUnifiedPrev=function(){const a=arr();if(!a.length)return;try{idx=(Number(idx||0)-1+a.length)%a.length}catch(e){}render()};
  window.vocabUnifiedNext=function(){const a=arr();if(!a.length)return;try{idx=(Number(idx||0)+1)%a.length}catch(e){}render()};
  window.vocabUnifiedMark=function(v){
    const x=at(); if(!x)return;
    try{
      if(isBipa()&&typeof window.bipaMarkFinal==='function'){window.bipaMarkFinal(v);setTimeout(render,0);return;}
      if(typeof window.mark==='function'&&window.mark!==window.vocabUnifiedMark){window.mark(v);setTimeout(render,0);return;}
    }catch(e){}
    try{const m=JSON.parse(localStorage.getItem('indo_mem')||'{}');m[x.word]=v;localStorage.setItem('indo_mem',JSON.stringify(m));}catch(e){}
    window.vocabUnifiedNext();
  };
  ...
  function enforce(){
    ...
    document.querySelectorAll('#vocabBox .flash,#vocabBox .cleanFlash,#vocabBox .bipaV7Card').forEach(n=>{if(!n.closest('.vocabUnifiedCard'))n.remove()});
  }

  const obs=new MutationObserver(()=>{const box=$('vocabBox');if(!box)return;if(box.querySelector('.flash,.cleanFlash,.bipaV7Card')&&!box.querySelector('.vocabUnifiedCard'))render();});
  function boot(){enforce();const box=$('vocabBox');if(box)obs.observe(box,{childList:true,subtree:false});setTimeout(()=>{enforce();render()},120);setTimeout(enforce,600);setTimeout(enforce,1500);}
  if(document.readyState==='complete')setTimeout(boot,0);else window.addEventListener('load',()=>setTimeout(boot,0),{once:true});
  window.addEventListener('vocab-library-ready',()=>setTimeout(()=>{enforce();render()},20));
  $('librarySelect')?.addEventListener('change',()=>setTimeout(()=>{enforce();render()},80));
